<template>
  <div class="trade-detail-wrap">
    ...
    <div class="detail-change">
      <div class="detail-copy">
        ...
        <b class="cursor" :class="{ copy: !isCopy }" v-clipboard:copy="detailInfo.txHash"
    ...
    <div class="information box-relative" v-loading="loading">
      ...
      ></i> -->
      <img v-if="detailInfo.txReceiptStatus == 1" src="../../assets/images/icon-success.svg"
        class="status-icon" />
      <img v-else-if="detailInfo.txReceiptStatus == 0" src="../../assets/images/icon-warning.svg"
        class="status-icon" />
      <!-- <i
      ...
      <List :title="detailTitle(detailInfo.txType)" :border="true" v-if="
        detailInfo.txType == '0' ||
        detailInfo.txType == '4' ||
        detailInfo.txType == '5' ||
      ...
      ">
        ...
        <Item :label="$t('tradeAbout.recipient')" v-if="detailInfo.txType != '4000'">
          ...
          <div class="isContract " v-if="detailInfo.receiveType == '1'">
            ...
            <router-link class="cursor normal ellipsis"
      ...
      ">
        <!-- 创建方 -->
        <Item :label="$t('tradeAbout.creator')" v-if="
          detailInfo.txType == '1' ||
          detailInfo.txType == '6' ||
          detailInfo.txType == '8' ||
        ...
        <!-- 执行方 -->
        <Item :label="$t('tradeAbout.executor')" v-if="
          detailInfo.txType == '2' ||
          detailInfo.txType == '7' ||
          detailInfo.txType == '9' ||
          detailInfo.txType == '21'
        ...
        <Item :label="$t('contract.contract')">
          ...
          <div class="isContract contract-fail" v-if="
        ...
        <Item :label="$t('tradeAbout.innerTransfer')"
          v-if="detailInfo.internalTransferParams && detailInfo.internalTransferParams.length > 0">
          <ul>
            <li class="tokens-item" v-for="(item, index) in detailInfo.internalTransferParams"
              :key="index">
              From
              <!-- 跳转参数都是innerContractAddr -->
              <router-link class="cursor blue ellipsis token-width"
                :to="getAddressUrl(item.from, item.fromType)">{{ item.from }}</router-link>
              to
              <router-link class="cursor blue ellipsis token-width"
                :to="getAddressUrl(item.to, item.toType)">{{ item.to }}</router-link>
              for
        ...
        <!-- erc20 tokens -->
        <Item :label="$t('tradeAbout.tokens')"
          ...
          <ul>
            <li class="tokens-item" v-for="(item, index) in detailInfo.erc20Params" :key="index">
              ...
              <!-- 跳转参数都是innerContractAddr -->
              <router-link class="cursor blue ellipsis token-width"
                :to="getAddressUrl(item.innerFrom, item.fromType)">{{ item.innerFrom }}</router-link>
              to
              <router-link class="cursor blue ellipsis token-width"
                :to="getAddressUrl(item.innerTo, item.toType)">{{ item.innerTo }}</router-link>
              for
        ...
        <Item :label="$t('tradeAbout.tokens')"
          ...
          <div class="table-721">
            ...
            <ul v-for="(item, index) in detailInfo.erc721Params" :key="index">
              ...
              <li>
                <router-link class="cursor blue ellipsis token-width"
                  :to="getAddressUrl(item.innerTo, item.toType)">{{ item.innerTo }}
                </router-link>
              </li>
              <li>
                <router-link class="cursor blue ellipsis token-width"
              ...
              <li>
                <img v-pic-preview class="token-pic" :src="
                  item.innerImage ||
        ...
        <Item :label="$t('tradeAbout.tokens')"
          ...
          <div class="table-721">
            ...
            <ul v-for="(item, index) in detailInfo.erc1155Params" :key="index">
              <li>
                <router-link class="cursor blue ellipsis token-width"
                  :to="getAddressUrl(item.from, item.fromType)">{{ item.from }}
                </router-link>
              </li>
              <li>
                <router-link class="cursor blue ellipsis token-width"
                  :to="getAddressUrl(item.to, item.toType)">{{ item.to }}
                </router-link>
              </li>
              <li>
                <router-link class="cursor blue ellipsis token-width"
      ...
      <List :title="detailTitle(detailInfo.txType)" :border="true"
        ...
        <Item :label="$t('tradeAbout.validator')">
          <router-link v-if="detailInfo.nodeName" class="cursor normal ellipsis"
            :to="getNodeUrl(detailInfo.nodeId)">{{ detailInfo.nodeName }}</router-link>
        ...
        <!-- 领取奖励数 -->
        <Item v-if="detailInfo.txAmount > 0 && detailInfo.txType == '1005'"
      ...
      ">
        ...
        <template v-if="detailInfo.txType != '2004'">
          ...
          <Item :label="$t('tradeAbout.PIPSN')">
            ...
            <a v-if="detailInfo.proposalUrl" class="cursor normal ellipsis"
              :href="detailInfo.proposalUrl" target="_blank" rel="noopener noreferrer">{{
                detailInfo.pipNum }}</a>
          ...
          <Item :label="$t('tradeAbout.proposalTitle')">
            <router-link class="cursor normal ellipsis" v-if="detailInfo.proposalTitle"
              :to="getProposalUrl(detailInfo.proposalHash)">{{ detailInfo.proposalTitle
              }}</router-link>
            <router-link class="cursor normal ellipsis" :to="getProposalUrl(detailInfo.proposalHash)"
              v-else-if="
  ...
  detailInfo.pipNum
        ...
        <Item :label="$t('tradeAbout.vote')" v-if="detailInfo.txType == '2003'">
          <span v-if="detailInfo.proposalOption == 2"
            class="green vote-status Gilroy-Bold">YEAS</span>
        ...
        <!-- 声明版本(版本声明特有) -->
        <Item :label="$t('tradeAbout.version')" v-if="detailInfo.txType == '2004'"
          :prop="detailInfo.declareVersion"></Item>
        <!-- 交易手续费 -->
        <Item :label="$t('tradeAbout.transactionFee')">
          <span>{{ detailInfo.actualTxCost | formatMoney }} LAT</span>
        </Item>
      ...
      ">
        ...
        <Item :label="$t('tradeAbout.validator')">
          ...
          <template v-else>
            <p v-if="detailInfo.txType != '3000'" class="cursor blue"
              @click="goNodeDetail(detailInfo.nodeId)">
              <span v-if="detailInfo.nodeName">{{ detailInfo.nodeName }}</span>
              <span v-else>Null</span>
            </p>
        ...
        <template v-if="detailInfo.txType == '1000' || detailInfo.txType == '1001'">
          ...
          <Item :label="$t('tradeAbout.identity')">
            <a v-if="detailInfo.externalId" class="cursor normal ellipsis"
              :href="detailInfo.externalUrl" target="_blank">{{ detailInfo.externalId }}</a>
            <a v-else>Null</a>
          </Item>
          <!-- 奖励账户(创建，编辑验证人) -->
        ...
        <!-- 质押数量（创建验证人，增加质押） -->
        <Item v-if="detailInfo.txType == '1000' || detailInfo.txType == '1002'"
        ...
        <template v-if="detailInfo.txType == '1003'">
          ...
          <Item :label="$t('tradeAbout.returnAmount')">
            ...
            <!-- 1： 退回中 -->
            <span class="letUp" v-if="detailInfo.redeemStatus == 1">({{ $t('tradeAbout.pend')
            }})</span>
            <!-- 2：退回成功 -->
          ...
          <!-- 预计到账区块（退出验证人特有） -->
          <Item class="return-amount" :label="$t('tradeAbout.returnBlock')"
            :prop="detailInfo.redeemUnLockedBlock"></Item>
        </template>
        <template></template>
        <!-- 交易手续费 -->
        <Item :label="$t('tradeAbout.transactionFee')">
      ...
      <List :title="$t('tradeAbout.information')" class="common-info" :border="true">
        ...
        <Item :label="$t('tradeAbout.status')">
          ...
          <span v-else-if="detailInfo.txReceiptStatus == 0" class="pink Gilroy-Bold">{{
...
export default {
  ...
  computed: {
    ...
    curExtraData() {
      ...
      const data = txType == 0 ? input : txInfo
      try {
        if (data && data.startsWith('0x') && this.curDataShowType === 'utf8') {
